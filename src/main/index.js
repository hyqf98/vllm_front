...
function setupIPCHandlers() {
  ...
  ipcMain.handle('data:write', async (event, moduleName, data) => {
    console.log('[IPC] data:write 被调用, moduleName:', moduleName)
    console.log('[IPC] data:write 数据长度:', JSON.stringify(data).length)
    try {
      const success = await dataManager.writeData(moduleName, data)
      console.log('[IPC] data:write 返回, success:', success)
      const result = { success }
      console.log('[IPC] data:write 返回对象:', result)
      return result
    } catch (error) {
      console.error('[IPC] data:write 异常:', error)
      const result = { success: false, error: error.message }
      console.log('[IPC] data:write 返回错误对象:', result)
      return result
    }
  })
  ...
  ipcMain.handle('data:delete', async (event, moduleName) => {
    try {
      const success = await dataManager.deleteData(moduleName)
      return { success }
    } catch (error) {
      return { success: false, error: error.message }
    }
  ...
  ipcMain.handle('data:clearAll', async () => {
    try {
      const success = await dataManager.clearAllDataAndDirectory()
      return { success }
    } catch (error) {
      return { success: false, error: error.message }
    }
